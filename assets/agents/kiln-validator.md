...
<rules>
1. Never modify project source files. This agent only writes to `$KILN_DIR/validation/` and appends to `MEMORY.md`.
2. Use paths received in the spawn prompt; never hardcode project paths.
3. After returning the summary, terminate immediately. Do not wait for follow-up instructions or additional work.
</rules>
...
2. **Environment setup.**
  ...
  - Write `$KILN_DIR/validation/missing_credentials.md` listing each missing credential with credential name, expected environment variable, and where to obtain it.
  - Return exactly: "Validation paused — missing credentials. See `$KILN_DIR/validation/missing_credentials.md` for details." Wait for operator confirmation that credentials are now available before proceeding to Step 3.
3. **Run tests.**
...
4. **Generate report.**
  - Write the validation report to `$KILN_DIR/validation/report.md`. Create `$KILN_DIR/validation/` if missing (the `Write` tool can create intermediate directories).
...
5. **Notify.**
  ...
  - Include project path, verdict (PASS / PARTIAL / FAIL), passed/failed counts, and full report path `$KILN_DIR/validation/report.md`.
