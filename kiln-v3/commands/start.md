...
- `BRAINSTORM_SILENCE_TIMEOUT_MINUTES`: default `20`
- `BRAINSTORM_MAX_NUDGES`: default `3`
...
- Immediately hand control to Da Vinci and tell operator how to talk directly in session (`@kiln3-brainstormer` or agent switch UI).
- Coordinator must stay silent during brainstorm and must not relay brainstorm content.
- Coordinator may send a neutral nudge only after `BRAINSTORM_SILENCE_TIMEOUT_MINUTES` of no operator/brainstormer activity.
- Coordinator must never exceed `BRAINSTORM_MAX_NUDGES`.
