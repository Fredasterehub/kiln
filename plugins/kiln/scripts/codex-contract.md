...
The request is JSON with `reviewer_model`, `reviewer_effort`, `law_hash`, `criteria`, and string arrays `paths`, `failures`, and `commands`. The caller supplies the same request, including `reviewer_model` and `reviewer_effort`, throughout one review/repair loop; both are named once in `data/tiers.json` (`roles["reviewer-gate"]` + `resolver`), never hardcoded here. `reviewer_model` is passed as `--model` but never enters the evidence packet. `reviewer_effort` is validated against `low|medium|high|xhigh` (absent defaults to `high`); `ultra` and any unknown value are rejected before any Codex spawn with the stdout fact `reviewer_effort_invalid` (exit 20, the rejected value never reaches Codex).

The transport invokes `codex exec --ephemeral --sandbox read-only --skip-git-repo-check -C <repo> --model <reviewer_model> -c model_reasoning_effort="<reviewer_effort>" --output-schema <review-schema.json> -o <temporary-output> -`, with the prompt supplied on stdin. A valid result is atomically renamed to the requested gate path. This follows the current official [noninteractive contract](https://developers.openai.com/codex/noninteractive/) and [`codex exec` reference](https://developers.openai.com/codex/cli/reference/); `--skip-git-repo-check` permits the locked historyless fixtures without widening the read-only sandbox.
