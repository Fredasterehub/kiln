...
- **Final archive gate**: Before `BUILD_COMPLETE`, thoth must answer `FINAL_ARCHIVE_CHECK` with `ARCHIVE_READY` and write `.kiln/archive/step-5-build/final-archive-readiness.md` containing build/run scope, milestone scope, `head_sha`, timestamp/sequence, and checked source archive paths. Missing milestone summaries, missing chunk assignment/TDD/review evidence, pending critical tmp artifacts, stale head metadata, or uncaptured known limitations block final close-out.
