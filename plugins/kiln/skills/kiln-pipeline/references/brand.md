...
4. **Description is for logging** — the Bash `description:` parameter does NOT control the collapsed header (the command path does — see lore-engine.md). Description is still set from the table below for logging, accessibility, and internal consistency.
...
## Bash Description (logging — does not control UI header)
