...
| `ALREADY_APPROVED: assignment_id={X} chunk_id={Y}; verdict at .kiln/archive/milestone-{N}/chunk-{M}/review.md` | Reviewer's idempotent reply to a duplicate REVIEW_REQUEST for an already-APPROVED assignment_id (recipient: builder). Reviewer also re-emits IMPLEMENTATION_APPROVED to krs-one (idempotent per Blocking Policy Rule 4). Procedure in critical-thinker.md / the-curator.md § APPROVED. | Reviewers (critical-thinker, the-curator) |
