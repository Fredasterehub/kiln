...
The kiln orchestrator (`.claude/agents/kiln-orchestrator.md`) executes this skill as a control contract. This skill defines ordering, transitions, retry budgets, pause conditions, and final closeout behavior.
...
3. Execute the current step for the current phase:
   ...
   b. `VALIDATE` — branch by `preferences.useTeams`:
      ...
      - `true` + `modelMode: "multi-model"`: consume `plan-validation-result` generated by the PLAN team and enforce gate pass/fail semantics.
   ...
   f. `RECONCILE` — run reconciliation protocol via `/kiln:reconcile` (`.claude/skills/kiln-reconcile/kiln-reconcile.md`).
...
- `true` + `modelMode: "multi-model"`: use explicit Teams scheduling for plan generation (and dependent validation) as defined below. Team/task identity and payload semantics must conform to `.claude/skills/kiln-teams/kiln-teams.md`.
...
#### Required output
`PLAN.md` must contain task packets with goals, ACs, files, dependencies, waves, and rollback context as defined by `.claude/skills/kiln-plan/kiln-plan.md`.
...
- `preferences.useTeams: true` + `modelMode: "multi-model"` — validator runs as dependent PLAN-team task (`phase-N:plan:validator`); VALIDATE consumes its sentinel output.
- `preferences.useTeams: true` + `modelMode: "claude-only"` — spawn `kiln-validator` with `PLAN.md` and relevant living docs; no PLAN-team sentinel is emitted in this mode.

Teams mode gate contract (`preferences.useTeams: true`, `modelMode: "multi-model"`):
...
- PLAN ran via non-Teams path, so no `plan-validation-result` sentinel was emitted.
- Spawn `kiln-validator` independently, same as the non-Teams VALIDATE path.
- This override ensures VALIDATE never waits for a sentinel that cannot exist.
...
- `true`: use explicit Teams wave scheduler below. Worktree/copy-back semantics are normative from `.claude/skills/kiln-teams/kiln-teams.md` and `.claude/skills/kiln-execute/kiln-execute.md`.
...
Before spawning the next stage's subagent, the orchestrator emits a transition message drawn from `.claude/skills/kiln-lore/kiln-lore.md`. This provides rhythm at each stage boundary.

**Protocol:**
1. Read the matching transition section from `.claude/skills/kiln-lore/kiln-lore.md` for the stage about to begin.
...
- Persist failure evidence using the error-escalation contract from `.claude/skills/kiln-core/kiln-core.md`:
...
### Terminal Outcomes
On `PASS`:
- Generate `.kiln/FINAL_REPORT.md` from `.claude/templates/FINAL_REPORT.md.tmpl`.
...
STATE.md follows the canonical Markdown schema defined in kiln-core. See `.claude/skills/kiln-core/kiln-core.md` section "STATE.md Canonical Schema" and `.claude/templates/STATE.md.tmpl` for the authoritative format. Do not use YAML or any alternative state representation.
