// codex-bridge.test.mjs — the Kiln Dev Protocol bridge (A4/A5/A6/A7) over the shared transport.
// REAL_* constants are verbatim local read-only captures from codex-cli 0.144.1 on 2026-07-14
// (.kiln-dev/v302/probes/): a model-unavailable turn.failure, an invalid-effort turn.failure, and
...
test('the A5 state machine classifies every defined shape and defaults to TRANSPORT', () => {
  const clean = inspectBridgeEvents(parseEvents(CLEAN_VERDICT_EVENTS))
  const mu = inspectBridgeEvents(parseEvents(REAL_MODEL_UNAVAILABLE))
...
test('bridge subcommand: the model-unavailable fingerprint triggers one gpt-5.5 fallback, ineligible for a Sol seat, primary artifacts preserved', () => {
  ...
  try {
    ...
    // DSGN-3: the primary (failed) attempt's artifacts survive under IMMUTABLE attempt-scoped names,
...
// ── D4: resume record-binding + posture recording (ADOPT-2 / ADOPT-6) ────────────────────────────

test('bridge: the receipt and ledger record the full posture tuple + out_prefix (ADOPT-6, DSGN-3)', () => {
  const files = bridgeSandbox()
  try {
...
test('bridge: --resume binds to the recorded thread and derives model + eligibility from the recorded row, never opts (ADOPT-2)', () => {
  const files = bridgeSandbox()
  try {
...
test('bridge: --resume refuses to escalate the recorded sandbox posture (ADOPT-6)', () => {
  const files = bridgeSandbox()
  try {
    runBridge(files, 'suppressed', ['--no-fallback'])
...
// ADOPT-12: the Fable-terminal instrument is a BOUND WRAPPER — the inner A2 envelope plus the
...
function gateFixture({ mode = 'verdict', diffContent = 'diff --git a/x b/x\n+one\n', bindLine = true, postureLine = 'read-only network=0 web=0' } = {}) {
  ...
  // postureLine drives the ADOPT-6 posture binding; the fixture's bridge always runs default read-only.
...
test('gate: a commission with no Diff-sha256 binding line is rejected (exit 12, ADOPT-5)', () => {
...
test('gate: the fable lane validates a Fable-authored bound wrapper with fable_main_session provenance and no receipt (ADOPT-12)', () => {
  const fx = gateFixture()
  try {
...
test('gate: the lane is a CLOSED ENUM — an unknown lane refuses, and a /codex/-substring false positive no longer misclassifies (AMB-R2-2)', () => {
  const fx = gateFixture()
  try {
...
test('gate: the convergence oracle precedes the APPROVED seal (ADOPT-7) — an unchanged finding set escalates even on APPROVED', () => {
  const fx = gateFixture({ mode: 'approved' })
  try {
    // an APPROVED verdict has empty findings → finding_set_key '[]'. Seed a prior round (same batch_id,
    // ADOPT-8b) with the same key.
...
test('gate: r2 rejection ALWAYS routes to confirm-each-or-escalate (ADOPT-8c) — microfix-r3 is never emitted at r2-time, even with a scope artifact present', () => {
  const fx = gateFixture()
  try {
    ...
    // NOT promote r2 to microfix-r3 — concurrence cannot pre-exist the verdict being routed (ADOPT-8c).
...
// ── r3 fix batch: the five surviving-finding invariants ──────────────────────────────────────────

test('gate: ADOPT-1 current-attempt freshness — a bridge-ledger VERDICT row whose input_sha256 does not match THIS commission+schema is refused (exit 12)', () => {
  const fx = gateFixture()
  try {
...
test('gate: ADOPT-1 route-ledger uniqueness — a duplicate (batch_id, round) route is refused (exit 12), so a mis-relayed replay cannot route twice', () => {
  const fx = gateFixture()
  try {
    const m = writeManifest(fx.dir)
...
test('gate: ADOPT-1 the duplicate check, route computation, and append are ONE locked critical section — racing same-round invocations produce exactly one row', async () => {
  const fx = gateFixture()
  try {
    const m = writeManifest(fx.dir)
    const args = [
...
test('gate: ADOPT-6 posture consistency — a commission whose declared posture disagrees with the receipt tuple is refused (exit 12), a matching one passes', () => {
...
test('gate: ADOPT-8 r3 is a scoped one-item confirmation — valid scope routes microfix-r3 (21), absent scope and an out-of-scope blocking set are refused (exit 12)', () => {
  const fx = gateFixture()
  try {
    const m = writeManifest(fx.dir)
    // round 3 with NO scope artifact is illegal (ADOPT-8d) — exit 12
...
test('gate: ADOPT-12 the Fable-terminal wrapper is bound to the current diff/batch/round — a wrong sha, batch, round, or a bare verdict is refused (exit 12)', () => {
  const fx = gateFixture()
  try {
...
test('check-handoff: the real templates pass their kind (incl. microfix now carrying Allowed tools / inputs), malformed bounces', () => {
  ...
  assert.equal(check(TPL('brief-microfix.md'), 'microfix').status, 0) // ADOPT-9: the field was added
  ...
  try {
    ...
    // ADOPT-9: the confirm instrument now REQUIRES a Scope heading (carrying Diff-sha256 + Sandbox
