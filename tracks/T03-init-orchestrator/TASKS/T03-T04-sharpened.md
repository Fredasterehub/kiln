You are GPT-5.3-codex operating in repo root `/tank/dump/DEV/kiln` (bash). Implement the following exactly.

## Task
Add a new skill markdown file at `skills/kiln-quick/kiln-quick.md` that defines the `/kiln:quick` slash-command skill: a lightweight single-pass mode for small, well-understood changes that **skips** brainstorm/roadmap/dual-model planning ceremony but **keeps** quality gates (sharpening, implementation, adaptive verification, regression testing, stub detection, atomic commit). Must include concrete “too big for quick mode” heuristics with specific thresholds.

## Acceptance Criteria (must be satisfied)
- AC-04 (LLM): Defines single-pass mode that skips brainstorm, roadmap, and dual-model planning. Retains sharpening, implementation, adaptive verification, and regression testing. Includes concrete heuristics for “too big for quick mode” with specific thresholds.
- File exists at `skills/kiln-quick/kiln-quick.md`.
- File length is ~150–220 lines (target ~200).

## Constraints
- Add only this file (no other file changes).
- The file must start with the exact YAML frontmatter below.
- The file must contain the exact top-level heading and the five required sections with the exact headings listed below.
- Keep the exact code blocks and quoted strings shown (including the `\"` escapes inside the inline command examples).

## Implementation
Create `skills/kiln-quick/kiln-quick.md` with the following content (copy verbatim):
...
- `test -f skills/kiln-quick/kiln-quick.md`
- `wc -l skills/kiln-quick/kiln-quick.md` (should be ~150–220 lines; adjust spacing/blank lines only if needed to fit the range)