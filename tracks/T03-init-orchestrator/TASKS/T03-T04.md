# T03-T04: Write /kiln:quick skill

## GOAL
Write the /kiln:quick slash command skill — a lightweight single-pass mode for small, well-understood changes that skips the full brainstorm/roadmap/dual-planning ceremony but retains quality gates.
...
## FILES
- path: skills/kiln-quick/kiln-quick.md
  action: add
  rationale: The /kiln:quick slash command — lightweight mode for small changes

## COMMANDS
- `test -f skills/kiln-quick/kiln-quick.md` — file exists
- `wc -l skills/kiln-quick/kiln-quick.md` — should be ~150-220 lines

## SUMMARY
Create the file `skills/kiln-quick/kiln-quick.md`. This is a Claude Code skill file loaded when a user types `/kiln:quick "description"`. It provides a streamlined single-pass implementation mode for small changes.

**Context for the implementer:** Kiln's full pipeline is: brainstorm -> roadmap -> per-track loop (plan -> validate -> execute -> E2E -> review -> reconcile). This is thorough but heavyweight — it's designed for building entire features or applications from scratch. For small, well-understood changes (bug fixes, minor features, config tweaks), users need a lighter path. `/kiln:quick` provides this: it skips the ceremony (brainstorm, roadmap, dual-model planning) but keeps the quality gates (verification, stub detection, regression testing). It's a single-pass: plan once, implement once, verify once.
...
Then write heading `# /kiln:quick — Quick Mode` followed by these sections:
...
### Section 2: Scope Guard — "Too Big for Quick Mode"

Write heading `## Scope Guard`. Content:
...
### Section 3: Quick Mode Pipeline

Write heading `## Pipeline`. Content:
...
Write heading `## State Tracking`. Content:
...
Write heading `## Examples`. Content:
...
## RISKS
- Scope heuristics may be too strict (rejecting valid quick changes) or too loose (allowing changes that should use full pipeline)
- Users may attempt to bypass by breaking large changes into multiple quick calls
- Mini-verify in quick mode must handle projects with no test runner configured
